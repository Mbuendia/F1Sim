...
7. **🌿 Ramas por Sprint (GitHub)**: Cada sprint incompleto cuenta con su rama específica (ej. `sprint/2.8`, `sprint/2.9`, `sprint/3`, `sprint/4`, `sprint/5`). Toda IA que empiece a trabajar con este proyecto debe cambiarse a la rama del sprint en curso para hacer las implementaciones allí. Un sprint no se considerará finalizado hasta que su rama tenga todos los cambios asociados completados y hayan sido validados por el usuario.
