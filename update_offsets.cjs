...
const offsets = {
    'catalunya': { start: 0.12, entry: 0.90, exit: 0.15 },
    ...
    'red_bull_ring': { start: 0.86, entry: 0.90, exit: 0.10 },
...
for (const [key, data] of Object.entries(offsets)) {
    ...
    if (match) {
        ...
        tsCode = tsCode.replace(match[0], `${key}: {${block}\n    },`);
    }
}
